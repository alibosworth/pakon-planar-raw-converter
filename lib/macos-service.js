...
// Build the "Run Shell Script" body the Quick Action runs over the selected
// folders. Each folder path arrives in "$@"; it must reach `pprc --dir` intact
// and un-injectable regardless of apostrophes, spaces, or shell metacharacters
// in the name.
function buildShellCommand(command) {
  // The path is passed to osascript as an argument ("$f") and read from argv
  // inside a *quoted* heredoc, so the shell never expands it into the AppleScript
  // source. AppleScript's `quoted form of` then shell-quotes it for `do script`,
  // making apostrophes, spaces, and shell metacharacters safe.
  return [
    'for f in "$@"',
    'do',
    "osascript - \"$f\" <<'APPLESCRIPT'",
    'on run argv',
    '\tset folderPath to item 1 of argv',
    '\ttell application "Terminal"',
    '\t\tactivate',
    '\t\tdo script "' + command + ' --dir " & quoted form of folderPath',
    '\tend tell',
    'end run',
    'APPLESCRIPT',
    'done',
...
function generateWorkflow(command) {
  ...
  var shellCommand = buildShellCommand(command);
...
<dict>
  ...
  try {
    ...
    if (major >= 26) {
      extensionsPath = 'System Settings > General > Login Items & Extensions > Extensions > By Category > Finder, then click the Info (ⓘ) icon';
    } else if (major >= 15) {
...
export { install, uninstall, buildShellCommand };
