...
test('last_run_config from a negative run records the colour spaces', () => {
  ...
  try {
    ...
    assert.equal(cfg.colorspaceInput, 'adobergb', 'input colour space should be recorded');
...
// The shipped colour defaults. Pinned here because they are the one setting that
// silently changes every output image, and because a saved profile's numbers are
// only meaningful in the spaces they were computed in. Changing these should be a
// deliberate act that updates this test, the help text, the readme, and the site.
test('a run with no colour flags uses adobergb input, acescg working, adobergb output', () => {
  const dir = tmpDir();
  const home = path.join(dir, 'home');
  fs.mkdirSync(home);
  try {
    ...
    const { status, stderr } = runCli(['--dir', inDir, '--dir-out', outDir], {
      env: { HOME: home, USERPROFILE: home },
    });
    assert.equal(status, 0, `negative run should succeed. stderr:\n${stderr}`);

    const cfg = JSON.parse(fs.readFileSync(path.join(home, '.pprc', 'last_run_config.json'), 'utf8'));
    assert.equal(cfg.colorspaceInput, 'adobergb');
    assert.equal(cfg.colorspaceWorking, 'acescg');
    assert.equal(cfg.colorspaceOutput, 'adobergb');

    // The run log is what a user reads back months later, so it has to agree.
    const log = fs.readFileSync(path.join(outDir, 'pprc_log.txt'), 'utf8');
    assert.match(log, /^ {2}colorspace-input: adobergb$/m);
    assert.match(log, /^ {2}colorspace-working: acescg$/m);
    assert.match(log, /^ {2}colorspace-output: adobergb$/m);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
