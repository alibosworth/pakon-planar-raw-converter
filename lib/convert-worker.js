...
// ITU-R BT.709 luminance coefficients (sRGB standard)
// Used to convert color scan to single-channel greyscale,
// values match ImageMagick's "-colorspace Gray"
var LUMA_R = 0.2126;
var LUMA_G = 0.7152;
var LUMA_B = 0.0722;
...
if (applyGamma) {
  ...
  for (var j = 0; j < interleaved.length; j++) {
    interleaved[j] = gammaLUT[interleaved[j]];
  }
}
...
if (mode === 'bw') {
  ...
  for (var i = 0; i < pixelCount; i++) {
    gray[i] = Math.round(
      LUMA_R * interleaved[i * 3] +
      LUMA_G * interleaved[i * 3 + 1] +
      LUMA_B * interleaved[i * 3 + 2]
    );
  }
  outputData = gray;
  outputChannels = 1;
